...
describe("SystemConfig", () => {
  it("includes the supported web AI chat endpoints", () => {
    expect(bundled.capture.endpoints).toEqual(
  ...
  it("rejects ambiguous path patterns and excessive expanded rules", () => {
    ...
    ambiguous.capture.endpoints[0].hosts[0].paths[0] = {
      match: "exact",
      value: "/v1/*",
    };
    expect(() =>
      parseSystemConfig(ambiguous, DEFAULT_SAFETY_POLICY, bundled.issuedAt),
    ).toThrow(/only for glob/);
        ...
        hosts: [
          {
            ...
            paths: Array.from({ length: 65 }, (_, index) => ({
              match: "exact",
              value: `/v${index}`,
            })),
          },
    ...
    expect(() =>
      parseSystemConfig(excessive, DEFAULT_SAFETY_POLICY, bundled.issuedAt),
    ).toThrow(/expand to at most 256/);
  });
  ...
  it("enables capture from config and applies local consent only to delivery", () => {
    ...
    const projections = createConfigProjections(config, {
  ...
  it("rejects unknown fields and clamps configured limits", () => {
    ...
    expect(parsed.capture.http.maxBodyBytes).toBe(
      DEFAULT_SAFETY_POLICY.captureLimits.maxHttpBodyBytes,
    );
    candidate.unexpected = true;
    expect(() =>
      parseSystemConfig(candidate, DEFAULT_SAFETY_POLICY, bundled.issuedAt),
    ).toThrow(/unknown/);
  });
  ...
  it("reapplies local consent when reload retains the current config", async () => {
    const config = structuredClone(bundled) as Record<string, any>;
    config.delivery = {
      ...config.delivery,
      enabled: true,
      endpoint: "https://collector.example/captures",
    };
    ...
    const manager = createConfigManager({
      ...
      consent: { load: async () => ({ deliveryEnabled }) },
      safetyPolicy: {
        ...DEFAULT_SAFETY_POLICY,
        allowedDeliveryOrigins: ["https://collector.example"],
      },
      now: () => bundled.issuedAt,
    });
    await manager.initialize();
    expect(manager.projections()?.delivery.enabled).toBe(true);

    deliveryEnabled = false;
    sourceAvailable = false;

    expect((await manager.reload()).status).toBe("retained-current");
    expect(manager.projections()?.delivery.enabled).toBe(false);
  });
  ...
  it("returns deeply immutable snapshots and rejects oversized input", () => {
    ...
    expect(() =>
      parseSystemConfig(oversized, DEFAULT_SAFETY_POLICY, bundled.issuedAt),
